...
import type { SpellbookState } from "~/server/domain/spellbook-types";
import { createSpellbookUsecase } from "~/server/usecases/spellbook-usecase";
...
describe("spellbook usecase", () => {
	...
	test("validates input fields", async () => {
		...
		expect(result.fieldErrors.castid).toBeDefined();
	});
	...
	test("deletes entry by id", async () => {
		...
		const result = await usecase.deleteSpellbookEntry("a");
		expect(result.ok).toBe(true);
		expect(memory.getState().entries).toHaveLength(0);
	});
});
