...
function sortByCastId(entries) {
    return [...entries].sort((a, b) => a.castid - b.castid);
}
function resolveCastIdConflicts(entries) {
    const usedCastIds = new Set();
    let maxCastId = entries.reduce((currentMax, entry) => Math.max(currentMax, entry.castid), 0);
    const reassignments = [];
...
export function createSpellbookUsecase(deps) {
    return {
        ...
        async saveSpellbookEntry(input) {
            ...
            const currentIndex = state.entries.findIndex((entry) => entry.id === nextEntry.id);
            const mode = currentIndex >= 0 ? "updated" : "created";
            ...
            const { entries: resolvedEntries, reassignments } = resolveCastIdConflicts(mutableEntries);
            await deps.spellbookRepository.saveSpellbookState({ entries: resolvedEntries });
        ...
        async deleteSpellbookEntry(id) {
            ...
            await deps.spellbookRepository.saveSpellbookState({ entries: sortByCastId(nextEntries) });
