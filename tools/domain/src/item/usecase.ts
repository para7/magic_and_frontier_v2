...
function toFieldErrors(nested: v.FlatErrors<typeof saveItemSchema>["nested"]): ItemFieldErrors {
  ...
  if (nested.enchantments?.[0]) fieldErrors.enchantments = nested.enchantments[0];
  if (nested.customModelData?.[0]) fieldErrors.customModelData = nested.customModelData[0];
